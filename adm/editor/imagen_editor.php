...
//Parametros Recibidos
    if (!empty($_POST)){
        ...
        $codigoAlbum = $_POST['codigoAlbum']; //Codigo del album al que pertence la imagen a cargar
...
*/
            $sql = "DELETE FROM album WHERE id=".$borrar;
        ...
        } elseif (!empty($_POST['imagenCargada'])) {//si no se trata de un borrado, y en cambio se trata de una insercion
            $codigo = $_POST['codigo'];
            $titulo = $_POST['titulo'];
            $url = $_POST['editor'];


            $inicioCortar = strstr($url, 'data:image/');
            $finCortar = strpos($inicioCortar, 'alt=');
            $imagen = substr ($inicioCortar, 0, $finCortar-2); //Codigo cortado
            $imagen = str_replace('data:image/jpeg;base64,', '', $imagen);
            $imagen = str_replace(' ', '+', $imagen);
            $imagen = base64_decode($imagen);   
            $file = '../../img/galeria/'. "$codigo" .'.jpg';
            ...
            $sql = "INSERT INTO foto_album (codigo, titulo, url, album)
            VALUES ('".$codigo."', '".$titulo."', '".$textoReemplazo."', '".$codigoAlbum."')";
    ...
    $mysqli->real_query("SELECT codigo FROM foto_album ORDER BY codigo DESC LIMIT 1");
    $resultado = $mysqli->use_result();
...
<div class="col-md-12 cuerpoNoticias">
    ...
    <form action="imagen_editor.php" method="post">
        <?php
        ...
        ?>
                        <div class="alert alert-success col-md-12" role="alert">
                          ...
                          ¡Excelente! La Foto se cre&oacute; con &eacute;xito.
                        </div>
        <?php
                    }else {
        ?>
                        <div class="alert alert-danger col-md-12" role="alert">
                          ...
                          <span class="sr-only">Error:</span>
                          Ocurri&oacute; un Error al intentar guardar la Foto .
                        </div>
        <?php
                    }
                }
        ...
        ?>
                        <div class="alert alert-success col-md-12" role="alert">
                          <span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span>
                          <span class="sr-only">Error:</span>
                          ¡Excelente! La Foto se ELIMIN&Oacute; con &eacute;xito.
                        </div>
        <?php
                    }else {
        ?>
                        <div class="alert alert-danger col-md-12" role="alert">
                          <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
                          <span class="sr-only">Error:</span>
                          Ocurri&oacute; un Error al intentar eliminar la Foto.
                        </div>
        <?php
                    }
                }
        ...
        <div class="col-md-12 titulo-adm">
            <center><h2>Foto Nueva</h2></center>
        </div>
        <div class="col-md-6">
                    <div class="form-group">
                        <input type="hidden" name="imagenCargada" value="<?php echo "1" ?>" />
                            <input type="hidden" name="codigo" value="<?php echo $codigo ?>" />
                            <input type="hidden" name="codigoAlbum" value="<?php echo $codigoAlbum ?>" />
        ...
        <div class="editor col-md-6">
              ...
              <textarea id="editor1" name="editor" placeholder="Ingrese aqui la foto..." rows="10"></textarea>
        </div>

        <div class="col-md-12 contenido-centrado">
                    <input type="submit" class="btn btn-primary col-offset-6" value="Cargar Foto"/>
        </div>
...
<div class="col-md-12 titulo-adm tabla-lista-noticias tabla-lista-noticias">
    <center><h2>Lista Fotos</h2></center>
</div>
<div class="col-md-12 cuerpoNoticias">
        <div class="col-md-12">
...
<?php
    ...
    $mysqli->real_query("SELECT f.codigo, f.titulo, f.url FROM foto_album f WHERE f.album = $codigoAlbum ORDER BY f.codigo DESC LIMIT 100");
    $resultado = $mysqli->use_result();
    $cont = 0;
    while ($fila = $resultado->fetch_assoc()) {
        $codigo = $fila['codigo'];
                    ...
                    <td>
                        <div style="float:left; margin-left: 5px;">
                            <form action="" method="post">
                                <input type="hidden" name="codigoBorrar" value="<?php echo $idFoto; ?>"/>
