...
<!-- JB -->
    ...
    <link href="dtp/bootstrap-datetimepicker.min.css" rel="stylesheet" media="screen">
<!-- /JB -->  
...
*/
            $sql = "DELETE FROM noticia WHERE id=".$borrar;
            ...
            $textoReemplazo = '<img style="" src="img/noticias/'. $codigo .'.jpg" class="img-fluid"/>';
            ...
            //Segunda Vuelta de busqueda de imagen
            $inicioCortar = strstr($stringAMostrar, 'data:image/');
            if(strlen($inicioCortar)  > 0){
                ...
                $imagen = str_replace(' ', '+', $imagen);
                $imagen = base64_decode($imagen);   
                $file = '../../img/noticias/'. "$codigo" .'a.jpg';
                file_put_contents($file, $imagen);

                //Reemplazar Imagen imagen//
                //Obtiene desde y hasta donde cortar, incluyendo <img
                $inicioReemplazar = strstr($stringAMostrar, '.jpg');
                $inicioReemplazar = strstr($inicioReemplazar, '<img');
                $finReemplazar = $finCortar = strpos($inicioReemplazar, '>');
                //se obtiene el texto a reemplazar
                $totalAReemplazar = substr ($inicioReemplazar, 0, $finReemplazar+1);
                //Se coloca el texto de REEMPLAZO
                $textoReemplazo = '<img src="img/noticias/'. $codigo .'a.jpg" style="max-width:300px" width="300" align="left" hspace="10"/>';
                //Se Reemplaza lo correspondiente
                $stringAMostrar = str_replace($totalAReemplazar, $textoReemplazo, $stringAMostrar);
            }
            ...
            //Tercera Vuelta de busqueda de imagen
            $inicioCortar = strstr($stringAMostrar, 'data:image/');
            if(strlen($inicioCortar)  > 0){
                ...
                $imagen = str_replace(' ', '+', $imagen);
                $imagen = base64_decode($imagen);   
                $file = '../../img/noticias/'. "$codigo" .'b.jpg';
                file_put_contents($file, $imagen);

                //Reemplazar Imagen imagen//
                //Obtiene desde y hasta donde cortar, incluyendo <img
                $inicioReemplazar = strstr($stringAMostrar, 'a.jpg');
                $inicioReemplazar = strstr($inicioReemplazar, '<img');
                $finReemplazar = $finCortar = strpos($inicioReemplazar, '>');
                //se obtiene el texto a reemplazar
                $totalAReemplazar = substr ($inicioReemplazar, 0, $finReemplazar+1);
                //Se coloca el texto de REEMPLAZO
                $textoReemplazo = '<img src="img/noticias/'. $codigo .'b.jpg" style="max-width:300px" width="300" align="left" hspace="10"/>';
                //Se Reemplaza lo correspondiente
                $stringAMostrar = str_replace($totalAReemplazar, $textoReemplazo, $stringAMostrar);
            }
            ...
            //Cuarta Vuelta de busqueda de imagen
            $inicioCortar = strstr($stringAMostrar, 'data:image/');
            if(strlen($inicioCortar)  > 0){
                ...
                $imagen = str_replace(' ', '+', $imagen);
                $imagen = base64_decode($imagen);   
                $file = '../../img/noticias/'. "$codigo" .'c.jpg';
                file_put_contents($file, $imagen);

                //Reemplazar Imagen imagen//
                //Obtiene desde y hasta donde cortar, incluyendo <img
                $inicioReemplazar = strstr($stringAMostrar, 'b.jpg');
                $inicioReemplazar = strstr($inicioReemplazar, '<img');
                $finReemplazar = $finCortar = strpos($inicioReemplazar, '>');
                //se obtiene el texto a reemplazar
                $totalAReemplazar = substr ($inicioReemplazar, 0, $finReemplazar+1);
                //Se coloca el texto de REEMPLAZO
                $textoReemplazo = '<img src="img/noticias/'. $codigo .'c.jpg" style="max-width:300px" width="300" align="left" hspace="10"/>';
                //Se Reemplaza lo correspondiente
                $stringAMostrar = str_replace($totalAReemplazar, $textoReemplazo, $stringAMostrar);
            }
            ...
            //Quinta Vuelta de busqueda de imagen
            $inicioCortar = strstr($stringAMostrar, 'data:image/');
            if(strlen($inicioCortar)  > 0){
                ...
                $imagen = str_replace(' ', '+', $imagen);
                $imagen = base64_decode($imagen);   
                $file = '../../img/noticias/'. "$codigo" .'d.jpg';
                file_put_contents($file, $imagen);

                //Reemplazar Imagen imagen//
                //Obtiene desde y hasta donde cortar, incluyendo <img
                $inicioReemplazar = strstr($stringAMostrar, 'c.jpg');
                $inicioReemplazar = strstr($inicioReemplazar, '<img');
                $finReemplazar = $finCortar = strpos($inicioReemplazar, '>');
                //se obtiene el texto a reemplazar
                $totalAReemplazar = substr ($inicioReemplazar, 0, $finReemplazar+1);
                //Se coloca el texto de REEMPLAZO
                $textoReemplazo = '<img src="img/noticias/'. $codigo .'d.jpg" style="max-width:300px" width="300" align="left" hspace="10"/>';
                //Se Reemplaza lo correspondiente
                $stringAMostrar = str_replace($totalAReemplazar, $textoReemplazo, $stringAMostrar);
            }
            ...
            //Se agregan barras para interpretacion de simbolos en el motor SQL
            $titulo = addslashes($titulo);
            $stringAMostrar = addslashes($stringAMostrar);

            $date = $_POST['fechaHoras'];

            $sql = "INSERT INTO noticia (codigo, titulo, cuerpo, categoria, destacado, fechaHora)
            VALUES ('".$codigo."', '".$titulo."', '".$stringAMostrar."', '".$categoria."', '".$destacado."', '".$date."')";

            if ($conn->query($sql) === TRUE) {
                $success = "si";
            } else {
                $success = "no";
                //echo $conn->die(mysql_error());
            }
        }
    }
    $conn->close();
...
<div class="col-md-12 cuerpoNoticias">
    ...
    <form action="editor.php" method="post">
        ...
        <div class="col-md-6">
                    ...
                    <input size="16" type="text" value="<?php echo $fechaHoraActual; ?>" readonly class="form_datetime form-control" name="fechaHoras"/>
...
<div class="col-md-12 titulo-adm tabla-lista-noticias tabla-lista-noticias">
    <center><h2>Lista de Noticias</h2></center>
</div>
<div class="col-md-12 cuerpoNoticias">
        <div class="col-md-12">
...
<?php
    ...
    $mysqli->real_query("SELECT n.id, n.codigo, n.titulo, n.fechaHora, c.descripcion FROM noticia n INNER JOIN categoria_noticia c ON c.id = n.categoria ORDER BY n.codigo DESC LIMIT 400");
    $resultado = $mysqli->use_result();
    $cont = 0;
    while ($fila = $resultado->fetch_assoc()) {
        $id = $fila['id'];
                ...
                <tr>
                    ...
                    <td>
                        <div style="float:left">
                            ...
                            <form action="../../modificar.php" method="post">
                                <input type="hidden" name="codigoModificar" value="<?php echo $codigo; ?>"/>
                                <input type="submit" name="modificar" class="btn btn-warning col-offset-6" value="Modificar"/>
                            </form>
                        </div>
                        <div style="float:left; margin-left: 5px;">
                            <form action="" method="post">
                                <input type="hidden" name="codigoBorrar" value="<?php echo $id; ?>"/>
...
<script type="text/javascript" src="dtp/bootstrap-datetimepicker.js" charset="UTF-8"></script>
<script type="text/javascript" src="dtp/bootstrap-datetimepicker.fr.js" charset="UTF-8"></script>
<script type="text/javascript">
